...
class MQLHomeViewModel: NSObject {
    ...
    var statusListDataModel = MQLHomeDataModel()    
    ...
    func getStatuses(isPullUp: Bool, completionHandler: @escaping ([String:AnyObject]?, NSError?) -> Void) -> (){
        ...
        let since_id = isPullUp ? 0 : statusListDataModel.statuses.first?.id ?? 0
        let max_id = isPullUp ? statusListDataModel.statuses.last?.id ?? 0 : 0
        ...
        NetworkRequestEngine.share.accessTokenRequest("https://api.weibo.com/2/statuses/home_timeline.json", parameters:parameters) { (value, error) in
            ...
            if error == nil {//解析数据
                ...
                if isPullUp == true {
                    self.statusListDataModel.statuses = self.statusListDataModel.statuses + array
                }else{
                    self.statusListDataModel.statuses = array + self.statusListDataModel.statuses
                }
